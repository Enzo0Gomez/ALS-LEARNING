...
function StudentContent({ user, section, onSectionChange }) {
    ...
    if (activeQuiz) {
        const questions = [...(activeQuiz.quiz_questions || [])].sort((a, b) => a.question_order - b.question_order);
        return (
            <section className="student-panel p-4 shadow sm:p-8 rounded-2xl bg-surface">
                ...
                <form onSubmit={submitQuiz} className="mt-6 space-y-5 sm:mt-8 sm:space-y-6">
                    {questions.map((question, index) => <fieldset key={question.id} className="student-quiz-question min-w-0 p-4 border rounded-xl sm:p-5 border-border"><legend className="px-2 font-bold text-ink">Question {index + 1}</legend>{question.image_url && <img src={question.image_url} alt="Question illustration" className="max-h-64 max-w-full mt-3 rounded-lg" />}<p className="mt-3 leading-7 wrap-break-word text-ink">{question.question}</p><div className="mt-4 space-y-2">{[...(question.quiz_choices || [])].sort((a, b) => a.choice_order - b.choice_order).map((choice) => <label key={choice.id} className="flex min-h-11 items-center gap-3 p-3 border rounded-lg cursor-pointer wrap-break-word border-border hover:bg-bg-alt"><input type="radio" className="shrink-0" name={`question-${question.id}`} value={choice.id} checked={answers[question.id] === choice.id} onChange={() => setAnswers({ ...answers, [question.id]: choice.id })} />{choice.choice_text}</label>)}</div></fieldset>)}
    ...
    if (section === "dashboard") return <StudentOverview user={user} loading={loading} totalProgress={totalProgress} completedModules={completedModules} modules={modules} attempts={attempts} todoItems={todoItems} announcements={announcements} onTodoClick={openTodo} />;
...
function ContentSection({ title, description, children }) { return <section className="student-panel min-w-0 p-4 shadow sm:p-8 rounded-2xl bg-surface"><p className="text-xs font-bold uppercase tracking-[0.15em] text-secondary sm:text-sm">Student Portal</p><h1 className="mt-2 text-2xl font-bold wrap-break-word text-primary sm:text-3xl">{title}</h1><p className="mt-3 leading-6 text-ink-soft">{description}</p><div className="mt-6 sm:mt-8">{children}</div></section>; }
