...
function StudentContent({ user, section }) {
    ...
    async function loadData() {
        ...
        // Remote data controls the loading state after the request completes.
    ...
    const todoItems = [
        ...modules.filter((module) => Number(progressByModule.get(module.id)?.progress_percentage || 0) < 100).slice(0, 3).map((module) => `Continue module: ${module.title}`),
        ...quizzes.filter((quiz) => !attempts.some((attempt) => attempt.quiz_id === quiz.id)).slice(0, 3).map((quiz) => `Take quiz: ${quiz.title}`),
        ...announcements.slice(0, 2).map((item) => `Read announcement: ${item.title}`),
    ].slice(0, 5);
